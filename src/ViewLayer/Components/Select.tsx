...
export const Select: React.FunctionComponent<ISelectArgs> = (
  ...
  const getInjectedAnimationToSelect = (props2: {
    ...
    let keyframesStyleArr = [
      `._animationIn_${componentId}_ { min-height: ${hInInit}rem; height: ${hInInit}rem; }`,
    ]
    ...
    iterator.forEach((item, i) => {
      if (i < size2) {
        ...
        keyframesStyleArr.push(
          ...[
            `._animationIn_${componentId}_${i} { animation-duration: ${delay2}s; animation-name: in_${componentId}_${i}; animation-direction: alternate; }`,
            `@-webkit-keyframes in_${componentId}_${i} {
            0% { min-height: ${hOut}rem; height: ${hOut}rem; } 100% { min-height: ${hIn}rem; height: ${hIn}rem; }
          }`,

            `._animationOut_${componentId}_${i} { animation-duration: ${delay2}s; animation-name: out_${componentId}_${i}; animation-direction: alternate; }`,
  ...
  const getAddAnimationClass = (
    ...
    let output = `_animationOut_${componentId}_${0}`

    if (!onBlur && animationRef.current === '_animationOut') {
      output = `_animationIn_${componentId}_${optionsLen}`
      animationRef.current = '_animationIn'
    } else if (!onBlur && animationRef.current === '_animationIn') {
      output =
        optionsLen <= sizeIn
          ? `_animationIn_${componentId}_`
          : `_animationIn_${componentId}_${optionsLen}`
      animationRef.current = '_animationIn'
    } else if (onBlur) {
      output = `_animationOut_${componentId}_${optionsLen}`
