...
interface GetInitializedFacebookOAuthInterface {
  (branch: string): void
}
 ...
 */
export const getInitializedFacebookOAuth: GetInitializedFacebookOAuthInterface =
  branch => {
    useEffect(() => {
      const scriptProps = {
      ...
      const makeDispatchAsyncWrappered = async () => {
        try {
          ...
          handleEvents(
            {},
            { typeEvent: 'SET_OAUTH_FB_SCRIPT_STATE', data: true }
          )
